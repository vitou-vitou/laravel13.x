            ...
            <div class="bg-white shadow-sm ring-1 ring-gray-200 sm:rounded-lg overflow-hidden">
                @if ($customers->isEmpty())
                    <div class="px-6 py-16 text-center">
                        ...
                        <a href="{{ route('customers.create') }}" class="mt-3 inline-block text-sm font-medium text-indigo-600 hover:text-indigo-500">Add the first one</a>
                    </div>
                @else
                            ...
                            @foreach ($customers as $c)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <a href="{{ route('customers.show', $c) }}" class="text-sm font-medium text-gray-900 hover:text-indigo-600">{{ $c->name }}</a>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $c->email }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate">{{ $c->address }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                        <a href="{{ route('customers.edit', $c) }}" class="text-indigo-600 hover:text-indigo-500">Edit</a>
