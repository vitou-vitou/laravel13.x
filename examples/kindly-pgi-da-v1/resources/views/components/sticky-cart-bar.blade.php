...
@if ($visible)
    ...
    >
        <div class="mx-auto flex max-w-7xl items-center justify-between gap-3">
            <div class="min-w-0">
                <p class="text-sm font-semibold text-stone-900">{{ $stickyCartCount }} {{ Str::plural('item', $stickyCartCount) }}</p>
                <p class="text-xs text-stone-500">${{ number_format($stickyCartTotalCents / 100, 2) }}</p>
            </div>
            <a href="{{ route('cart.index') }}" class="btn-brand min-h-11 shrink-0 px-6">View cart</a>
        </div>
    </div>
@endif
