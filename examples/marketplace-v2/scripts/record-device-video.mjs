/**
 * Record marketplace-v2 mobile walkthrough with real Playwright device profile
 * (UA, touch, deviceScaleFactor) — not viewport-only resize.
 *
 * Usage (from examples/marketplace-v2):
 *   npm install
...
const outFile = path.join(videosDir, `marketplace-v2-home-${slug}-native.webm`);
const url = process.env.APP_URL ?? 'http://marketplace-v2.test';
