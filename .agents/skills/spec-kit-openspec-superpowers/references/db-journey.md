...
Type the **work** only. Agent expands quote (list+pdf+view+create+edit) then policy wire. Default = **all 13 catalog journeys** (10 live + 3 WIP). WIP skips create-quote. Extra waves only if `w2`/`w3` or another `do:`. See `.cursor/skills/7pj-wave/SKILL.md`.

## Scope

**In scope (13 journeys):** live Marine `0189`+twin `0206` · `0190` EEI · `0191` Burglary · `0192` Money · `0193` Plate · `0194` CAR · `0195` Bond · `0196` PI · `0197` BBB · `0198` D&O · `0199` EAR · WIP `0201` TCI · `0202` FGI.

Source: `resources/data/direct-book-lines.json`. Staff checklist: `.cursor/skills/pgi-core/references/journey-7pj.md` (**39** cells = 30 live + 9 WIP).

**Verdict:** Keep slash **`7pj`**. `0206` = Marine twin of `0189`, not a new journey. BBB/D&O/EAR **are** live journeys. WIP **is** on 7pj (existing docs). **No `8pj` rename.** Count = JSON.
...
| **Endt** | L3 in scope for all 13 journeys (or only if user said `+endt` / `endt-only` when narrowing) |
...
| Flag | Meaning |
|------|---------|
| `marine` / `0189` / `0206` / `bbb` / `0197` / `dno` / `0198` / `ear` / `0199` / `eei` / `0190` / `tci` / `0201` / `fgi` / `0202` / `live` / `wip` / product name | One journey or band (Marine covers both SM codes) |
