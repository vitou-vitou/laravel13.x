...
Asked about the 7pj **user** journey (quote → policy → endorsement as staff see it)? Read [references/journey-7pj.md](references/journey-7pj.md). PDF / signature files from a UAT URL: same file, **PDF fixture**.
...
Scope is locked to **Direct Book catalog** (`direct-book-lines.json`). Slash **`7pj`**. Full rule: `.cursor/rules/02-pl-seven-product-scope.mdc`. Staff journey: [references/journey-7pj.md](references/journey-7pj.md) — **13 journeys × 3 levels = 39**.

| Code(s) | Line | Sale |
|---|---|---|
| `0189` + twin `0206` | Marine Cargo (one journey) | live |
| `0191` | Burglary | live |
| `0192` | Money | live |
| `0193` | Plate Glass | live |
| `0194` | Construction All Risks | live |
| `0195` | Bond | live |
| `0196` | Professional Indemnity | live |
| `0197` | Bankers Blanket Bond | live |
| `0198` | Directors & Officers | live |
| `0199` | Erection All Risks | live |
| `0190` | Electronic Equipment | live |
| `0201` | Trade Credit | wip |
| `0202` | Fidelity Guarantee | wip |
...
Journey shorthand `7pj` = L1 Quote → L2 Policy → L3 Endorsement. Default wave = **13 journeys**. WIP: no new quote.
