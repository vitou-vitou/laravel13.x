...
Building Direct Book lines through Quote → Policy → Endorsement (slash `7pj`; **11** live journeys). See `journey-7pj.md`.

Marine Cargo (`0189`/`0206` twin), Burglary `0191`, Money `0192`, Plate Glass `0193`, CAR `0194`, Bond `0195`, PI `0196`, BBB `0197`, D&O `0198`, EAR `0199`, EEI `0190`. WIP: TCI `0201`, FGI `0202`.

Design choice: live lines share one helper folder — named `burglary/`, since Burglary was first. `Plan.vue` and `Premium.vue` are config-driven per product code. A fix in `burglary/` can hit every live line.
