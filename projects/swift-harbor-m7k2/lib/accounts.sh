#!/usr/bin/env bash
# Account store (accounts.json) — Shell 100%.
set -euo pipefail
...
accounts_upsert() {
  ...
  tmp="$(mktemp)"
  # Minimal JSON merge via python one-liner avoided — use node if present, else append pattern
...
if (i >= 0) {
  ...
  list.push({ email, password, createdAt: now, posts: 0 });
}
